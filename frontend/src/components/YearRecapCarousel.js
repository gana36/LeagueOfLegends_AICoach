...
const YearRecapCarousel = ({ puuid, playerName }) => {
  ...
  const fetchNarrative = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:8000/api/analytics/year-narrative', {
        method: 'POST',
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 relative overflow-hidden">
        ...
        <div className="text-center z-10">
          <div className="relative inline-block mb-8">
            ...
            <div className="absolute top-0 left-0 animate-spin rounded-full h-20 w-20 border-r-4 border-l-4 border-pink-400" style={{animationDirection: 'reverse'}}></div>
          </div>
          <h2 className="text-white text-3xl font-bold mb-2">Crafting Your Story</h2>
          <p className="text-white/70 text-lg">Analyzing your year on the Rift...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-red-900 to-gray-900">
        <div className="text-center text-white max-w-md px-6">
          ...
          <p className="text-xl mb-2">Unable to generate your narrative</p>
          <p className="text-white/60 text-sm">{error}</p>
          <button
            onClick={fetchNarrative}
            className="mt-8 px-6 py-3 bg-white/20 hover:bg-white/30 rounded-full font-semibold transition-all"
          >
            Try Again
          </button>
        </div>
...
const HeroCard = ({ card }) => (
  <div className="text-center text-white space-y-12 animate-fade-in-up relative">
    ...
    <div className="space-y-6 relative z-10">
      ...
      <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black leading-tight tracking-tight text-shadow-lg uppercase" style={{textShadow: '0 0 20px rgba(200, 155, 60, 0.5)'}}>
        {card.title}
      </h1>
      <p className="text-2xl sm:text-3xl md:text-4xl text-[#C89B3C] font-bold tracking-widest uppercase">
        {card.subtitle}
      </p>
    </div>
    ...
    <div className="mt-16 relative z-10">
      <div className="relative inline-block group">
        ...
        <div className="relative bg-gradient-to-b from-[#0a1428] to-[#1b263b] rounded-lg p-10 md:p-12 border-2 border-[#C89B3C] shadow-2xl">
          <div className="text-6xl md:text-7xl mb-4 drop-shadow-[0_0_10px_rgba(200,155,60,0.5)]">{card.primary_stat.icon}</div>
          <div className="text-6xl md:text-7xl font-black mb-2 text-[#C89B3C]" style={{textShadow: '0 0 20px rgba(200, 155, 60, 0.8)'}}>{card.primary_stat.value}</div>
          <div className="text-xl md:text-2xl text-white/90 font-bold tracking-[0.1em] uppercase">
    ...
    {card.secondary_stats && (
      <div className="flex justify-center gap-12 md:gap-16 mt-16 relative z-10">
        {card.secondary_stats.map((stat, idx) => (
          <div key={idx} className="text-center group">
            <div className="text-4xl md:text-5xl font-black mb-2 text-[#C89B3C] group-hover:scale-110 transition-transform" style={{textShadow: '0 0 10px rgba(200, 155, 60, 0.5)'}}>{stat.value}</div>
            <div className="text-base md:text-lg text-white/80 font-bold uppercase tracking-wider">{stat.label}</div>
          </div>
        ))}
      </div>
    )}
...
const ChampionShowcaseCard = ({ card }) => (
  <div className="text-center text-white space-y-8">
    <h2 className="text-5xl font-bold mb-8">{card.title}</h2>

    <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-12 border border-white/20">
      <div className="text-6xl font-bold mb-4">{card.main_champion.name}</div>
      <div className="text-3xl text-white/70 mb-2">{card.main_champion.games} games</div>
      <div className="text-xl text-white/50">{card.main_champion.percentage}% of your matches</div>
    </div>

    <div className="mt-8 space-y-4">
      <p className="text-white/60">Your Top Champions:</p>
      <div className="flex justify-center gap-6">
        {card.top_champions.slice(0, 3).map((champ, idx) => (
          <div key={idx} className="bg-white/5 rounded-xl p-4 border border-white/10">
            <div className="text-xl font-semibold">{champ.name}</div>
            <div className="text-sm text-white/50">{champ.games} games</div>
          </div>
        ))}
      </div>
    </div>
...
const TimeStatsCard = ({ card }) => (
  <div className="text-center text-white space-y-8">
    <h2 className="text-5xl font-bold mb-12">{card.title}</h2>

    <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-12 border border-white/20">
      <div className="text-7xl font-bold mb-4">{card.main_stat.value}</div>
      <div className="text-3xl text-white/70 mb-2">{card.main_stat.label}</div>
      <div className="text-lg text-white/50">{card.main_stat.sublabel}</div>
    </div>
...
// Support Stats Card (Vision)
const SupportStatsCard = ({ card }) => (
  <div className="text-center text-white space-y-8">
    <h2 className="text-5xl font-bold mb-12">{card.title}</h2>

    <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-12 border border-white/20">
      <div className="text-6xl mb-4">{card.main_stat.icon}</div>
      <div className="text-6xl font-bold mb-2">{card.main_stat.value}</div>
      <div className="text-2xl text-white/70">{card.main_stat.label}</div>
    </div>
    ...
    <div className="space-y-6 mt-12">
      {card.secondary_stats.map((stat, idx) => (
        <div key={idx} className="bg-white/5 rounded-xl p-6 border border-white/10">
          ...
          <div className="text-lg text-white/70 mb-1">{stat.label}</div>
...
// Highlight Card (Best Month)
const HighlightCard = ({ card }) => (
  <div className="text-center text-white space-y-8">
    <div>
      <h2 className="text-5xl font-bold mb-4">{card.title}</h2>
      <p className="text-3xl text-white/70">{card.month}</p>
    </div>

    <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-12 border border-white/20 mt-12">
      <p className="text-2xl text-white/80 mb-8">{card.description}</p>
      {card.stats && (
        <div className="grid grid-cols-2 gap-6">
          <div>
            <div className="text-4xl font-bold">{card.stats.games}</div>
            <div className="text-white/60">Games Played</div>
          </div>
          <div>
            <div className="text-4xl font-bold">{card.stats.wins}</div>
