    ...
    <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border border-cyan-500/20 shadow-lg shadow-cyan-500/5 h-full">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <RadarChart data={radarData}>
          ...
          <Radar name="Player A" dataKey="playerA" stroke="#06b6d4" fill="#06b6d4" fillOpacity={0.5} strokeWidth={2} />
          {comparisonMode && (
            <Radar name="Player B" dataKey="playerB" stroke="#e879f9" fill="#e879f9" fillOpacity={0.4} strokeWidth={2} />
          )}
          {!comparisonMode && (
      ...
      <div className="mt-4 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-400">Player A</span>
        </div>
        {comparisonMode ? (
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-fuchsia-400" />
            <span className="text-sm text-slate-400">Player B</span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
